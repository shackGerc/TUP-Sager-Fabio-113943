...
namespace Problema_1_Unidad_1
{
    public partial class frmNuevaCarrera : Form
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            int cod_carrera = DBHelper.ObtenerInstancia().InsertarCarreraConSP("pa_insertar_carrera",
                carrera);
            if (cod_carrera != -1)
            {
                if (carrera.DetallesCarrera.Count != 0)
                {
                    if(!DBHelper.ObtenerInstancia().InsertarDetallesCarreraConSP("pa_insertar_detalleCarrera", carrera))
                    {
